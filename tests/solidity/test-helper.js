...
function checkTestEnv() {
  const argv = yargs(hideBin(process.argv))
    ...
    .example(
      "$0 --network ethermint --allowTests=test1,test2",
      "run only test1 and test2 using ethermint network"
    )
    .help("h")
    .alias("h", "help")
    ...
    .boolean("verbose-log")
    .describe("verbose-log", "print aircosmicd output, default false").argv;
...
function setupNetwork({ runConfig, timeout }) {
  ...
  const spawnPromise = new Promise((resolve, reject) => {
    const aircosmicdProc = spawn("./init-test-node.sh", {
      cwd: __dirname,
      stdio: ["ignore", runConfig.verboseLog ? "pipe" : "ignore", "pipe"],
    });

    logger.info(`Starting aircosmicd process... timeout: ${timeout}ms`);
    if (runConfig.verboseLog) {
      aircosmicdProc.stdout.pipe(process.stdout);
    }
    aircosmicdProc.stderr.on("data", (d) => {
      const oLine = d.toString();
      if (runConfig.verboseLog) {
        process.stdout.write(oLine);
      }
      if (oLine.indexOf("Starting JSON-RPC server") !== -1) {
        logger.info("aircosmicd started");
        resolve(aircosmicdProc);
      }
    });
  });

  const timeoutPromise = new Promise((resolve, reject) => {
    setTimeout(() => reject(new Error("Start aircosmicd timeout!")), timeout);
  });
