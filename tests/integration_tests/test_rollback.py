...
def custom_ethermint(tmp_path_factory):
    ...
    cmd = [
        "nix-build",
        "--no-out-link",
        Path(__file__).parent / "configs/broken-aircosmicd.nix",
    ]
    print(*cmd)
    broken_binary = (
        Path(subprocess.check_output(cmd).strip().decode()) / "bin/aircosmicd"
    )
    print(broken_binary)
...
def test_rollback(custom_ethermint):
    ...
    update_node_cmd(custom_ethermint.base_dir, "aircosmicd", 1)
