...
"$PWD"/build/aircosmicd keys add $KEY --keyring-backend test --algo "eth_secp256k1"

# Set moniker and chain-id for Ethermint (Moniker can be anything, chain-id must be an integer)
"$PWD"/build/aircosmicd init $MONIKER --chain-id $CHAINID
...
"$PWD"/build/aircosmicd add-genesis-account "$("$PWD"/build/aircosmicd keys show "$KEY" -a --keyring-backend test)" 100000000000000000000aphoton,10000000000000000000stake --keyring-backend test

# Sign genesis transaction
"$PWD"/build/aircosmicd gentx $KEY 10000000000000000000stake --amount=100000000000000000000aphoton --keyring-backend test --chain-id $CHAINID

# Collect genesis tx
"$PWD"/build/aircosmicd collect-gentxs

# Run this to ensure everything worked and that the genesis file is setup correctly
"$PWD"/build/aircosmicd validate-genesis

# Start the node (remove the --pruning=nothing flag if historical queries are not needed) in background and log to file
"$PWD"/build/aircosmicd start --pruning=nothing --rpc.unsafe --json-rpc.address="127.0.0.1:8545" --keyring-backend test > aircosmicd.log 2>&1 &

# Give aircosmicd node enough time to launch
sleep 5
...
#PRIVKEY="$("$PWD"/build/aircosmicd keys export $KEY)"
...
# After tests
# kill test aircosmicd
echo "going to shutdown aircosmicd in 3 seconds..."
sleep 3
pkill -f "ethermint*"